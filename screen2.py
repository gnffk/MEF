...
def InitGraph(window):
    ...
    ListBoxScrollbar = Scrollbar(listbox_frame1, bg='#efc376')
    ListBoxScrollbar.pack(side=RIGHT, fill=Y)

    search_listbox = Listbox(listbox_frame1, font=(font_name, 10), bg='#efc376', yscrollcommand=ListBoxScrollbar.set)
    search_listbox.pack(side=LEFT, fill=BOTH, expand=True)
